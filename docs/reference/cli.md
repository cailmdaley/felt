...
| `felt shuttle sessions <fiber\|session-uuid>` | Discover the fiber's composite Shuttle session ledger by UID, including historical paths, lifecycle events, hosts, harnesses, staleness, per-session `START`/`END`/`SIZE`/`EXIT`, and transcript availability. A session UUID or `--commit <sha>` reverse-resolves the owning fiber and its disposition through the ledgers; `--recent[=N]` lists the newest N sessions (default 20) across the whole store instead; `--materialize [--dir <d>]` resolves every available transcript to an ordinary local file and writes a `manifest.json` (`--json` for the structured result) |
