...
A good constitution makes worker decisions inferable from system purpose + codebase rather than gating on the human; spend the authoring time on Desired State and Context. The first dispatch's worker creates `report.html` from `references/report-template.html` in this skill; subsequent dispatches edit it in place.
