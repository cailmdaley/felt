...
The human's surfaces: the **board** (served by the daemon at `:4000`) is a pure view over the same fibers and sessions, with four hotkey-switchable tabs — **Desk** (the kanban, described below), **Day** (fiber lanes over a 6am→6am axis, one lane per fiber with two clocks — solid marks for human activity, wash for agent), **Week** (past days as rows of ink rasters, today marked by a gold seam, future rows hollow), and **Chronicle** (fibers as multi-day lifelines across calendar days — drag across days to draw a cycle or era). Use Desk to stash ideas, launch dispatches, steer workers, review what comes back; use Day/Week/Chronicle to see where time actually went. The **agent registry** (`felt shuttle agents`) maps a fiber's `shuttle.agent` to the CLI + model each dispatch runs. The daemon ships with felt; the operator guide lives at <https://cailmdaley.github.io/felt/> (AGENTS.md in the felt repo, for contributors).
