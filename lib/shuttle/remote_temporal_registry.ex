defmodule Shuttle.RemoteTemporalRegistry do
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, state) when is_reference(ref) do
    case Map.pop(state.tasks, ref) do
      ...
      {name, tasks} ->
        ...
        entries = RegistryCommon.record_failure(state.entries, name, reason, DateTime.utc_now())
  ...
  defp start_fetch(%State{} = state, %Remote{} = remote, entry, window) do
    ...
    entries = RegistryCommon.stamp_attempt(state.entries, remote, &initial_entry/1)
    ...
    case Map.pop(state.tasks, ref) do
      ...
      {_name, tasks} ->
        case Map.get(state.entries, name) do
          # See `Shuttle.RemoteFiberRegistry.record_task_result/4`: a fleet
          # reload dropped this remote mid-fetch, so the late result is dropped
          # rather than resurrecting a phantom entry nothing would evict.
          nil ->
            %{state | tasks: tasks}
  ...
  defp fetch(client, url, etag, timeout_ms, opts) do
    ...
    case RegistryCommon.conditional_get(client, url, etag, timeout_ms) do
      ...
      {:ok, status, headers, body} when status in 200..299 ->
        decode(body, key, also, RegistryCommon.header_value(headers, "etag"))
  ...
  defp persist(dir, name, entry),
    do: RegistryCommon.persist(dir, name, encode_entry(entry), "RemoteTemporalRegistry")
