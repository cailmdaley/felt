...
// gitignore-litter fix: a store initialized BEFORE F4 already has a
// felt-generated .gitignore missing the "*.md.lock" line (Storage.Init only
// writes .gitignore when one is entirely absent, so such a store never
// self-heals on its own). Storage.LockFiber must backfill that line so lock
// sidecars don't accumulate as untracked files.
func TestEnsureGitignoreCoversLocks_AppendsToExistingGeneratedFile(t *testing.T) {
	...
	preF4 := "# Generated by felt — local index + sync coordination, regenerate from fibers\n" +
		"index.db\nindex.db-wal\nindex.db-shm\nindex-sync.lock\nindex-sync.request\n"
	gitignorePath := filepath.Join(st.root, GitignoreName)
	if err := os.WriteFile(gitignorePath, []byte(preF4), 0644); err != nil {
		t.Fatalf("seeding pre-F4 .gitignore: %v", err)
	}
