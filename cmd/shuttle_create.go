...
Started (status:active) it LOOPS — dispatched and re-dispatched on every worker
exit — until a human parks it again or a worker closes it. Perennial: you park
it, you don't delete it.`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		reg, err := shuttle.LoadAgentRegistry()
		if err != nil {
		...
		fmt.Printf("pinned %s (parked on the strip; start it to loop)\n", args[0])
		fmt.Printf("  host: %s\n", block.Host)
		if block.Agent != "" {
			fmt.Printf("  agent: %s\n", block.Agent)
		}
